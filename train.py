...
def train2(args, data_train, data_test, model_style, model_loss, optimizer, schedular, device, style_img, mse_loss,
           msesum_loss,
        ...
        for id, sample in enumerate(data_bar):
            ...
            for i in range(len(output_style)):
                ...
                style_loss += STYLE_WEIGHTS[i] * (
                            mse_loss(output_style_g, output1_g) + mse_loss(output_style_g, output2_g))
            ...
            reg_loss = reg_loss + (
                        torch.sum(torch.abs(output_img2[:, :, :, :-1] - output_img2[:, :, :, 1:])) + torch.sum(
                    torch.abs(
...
def main():
    ...
    if args.phase == 'train' or args.phase == 'Train':
        # a=1
        train2(args, data_train, data_test, model_style, model_loss, optimizer, schedular, device, style_img, mse_loss,
               msesum_loss,
